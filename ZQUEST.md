...
3. nexttick的原理
4. data为什么使用函数，为什么会全局污染
    1. 因为组件是可以被复用的，如果使用对象的话，在我们使用的过程中如果改变data中的数据，将会影响到所有复用的组件。为了保证不会相互影响，使用函数创建私有的作用域，保证数据不会被相互影响。
    2. 每次实例化组件调用data函数，返回一个新的对象，使其组件具有单独的数据对象。对于原型链上来说，就是创建的属性方法必须是一个函数，如果不是函数，再多次引用的时候会出现数据关联的情况。
    上面说的全局污染不太对，主要点没有答上。
5. slot的原理
6. 手写发布订阅模式
7. vue的响应式原理具体是怎么实现的
